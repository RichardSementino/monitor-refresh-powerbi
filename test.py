...
class MonitorDadosEPTV:
    def __init__(self, conn_str=None, container=None, arquivo=None, local_mode=False, local_file=None):
    ...
    def carregar_workbook(self):
        if self.local_mode:
            print(f"📥 Carregando arquivo local: {self.local_file}")
            self.wb = openpyxl.load_workbook(self.local_file)
        else:
    ...
    def salvar_workbook(self):
        if self.local_mode:
            print(f"📤 Salvando arquivo local: {self.local_file}")
            self.wb.save(self.local_file)
            print("✅ Arquivo local salvo com sucesso.")
        else:
    ...
    def obter_status_e_detalhe(self, workspace_id, dataset_id, token):
        ...
        try:
            ...
            if response.status_code != 200:
                return "ERRO_API", f"HTTP {response.status_code}: {response.text}", None, None

            dados = response.json()

            if not dados.get("value"):
                return "SEM_HISTORICO", "Sem histórico de refresh.", None, None

            ultimo = dados["value"][0]
            status_api = ultimo.get("status")
            ...
            msg.append(f"Status: {status_api}")

            if start:
                msg.append(f"Início: {start}")
            if end:
                msg.append(f"Fim: {end}")
            ...
            if detalhe:
                ...
                msg.append("Detalhe:")
                msg.append("Erro sem detalhe retornado pela API.")

            return "Erro", "\n".join(msg), data_refresh, hora_refresh

        except Exception as e:
            return "ERRO_API", f"Falha comunicação API: {e}", None, None

    # ==============================
    # ATUALIZA MONITORAMENTO
    # ==============================
    def atualizar_monitoramento(self, painel, status):
        # Só escreve no monitoramento diário se for OK ou Erro real do dia
        if status not in ["OK", "Erro"]:
            print(f"⏭ Monitoramento diário ignorado para {painel} - status: {status}")
            return
    ...
    def registrar_erro_dashboard(self, painel, mensagem):
        if not mensagem:
            mensagem = "Erro sem detalhe retornado pela API"

        hoje_dt = datetime.now().date()
        hoje_pt = hoje_dt.strftime("%d/%b/%y").lower()
        weekday_pt = hoje_dt.strftime("%A")

        next_row = self.proxima_linha_vazia(self.ws_erros)
        ...
        print(f"DEBUG ERRO -> linha={next_row}, painel={painel}, mensagem={mensagem}")
    # ==============================
    # CHECK DE LINHAS EXCEL 
    # ==============================
    def proxima_linha_vazia(self, ws, col_ref=1, start=3):
        row = start
        while ws.cell(row=row, column=col_ref).value not in (None, "", " "):
            row += 1
        return row
    # ==============================
    # ATUALIZA CATÁLOGO REPORTS
    # ==============================
...
if __name__ == "__main__":
    ...
    # 👉 Modo local para facilitar testes
    LOCAL_MODE = True
    LOCAL_FILE = r"C:\Users\vcp19001596\Desktop\proj.automacao\Catálogos de Gestão_Dados_copia.xlsx"

    if not LOCAL_MODE and not all([MINHA_CONEXAO, T_ID, C_ID, C_SECRET]):
        print("❌ Variáveis de ambiente não configuradas corretamente.")
        exit()

    if LOCAL_MODE and not all([T_ID, C_ID, C_SECRET]):
        print("❌ Variáveis PBI_TENANT_ID, PBI_CLIENT_ID e PBI_CLIENT_SECRET devem estar definidas.")
        exit()
